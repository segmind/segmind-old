...
import operator
import sqlparse
from sqlparse.sql import Comparison, Identifier, Statement, Token
from sqlparse.tokens import Token as TokenType

from segmind.entities import RunInfo
...
class SearchUtils(object):
    ...
    def _strip_quotes(cls, value, expect_quoted_value=False):
        ...
        Values of type strings are expected to have quotes. Keys containing
        special characters are also expected to be enclose in quotes.
        """
    ...
    def _valid_entity_type(cls, entity_type):
        ...
        if entity_type in cls._ALTERNATE_PARAM_IDENTIFIERS:
            ...
            # one of ("metric", "parameter", "tag", or "attribute") since it a
            # valid type
            return entity_type
    ...
    def _get_identifier(cls, identifier, valid_attributes):
        ...
        if identifier == cls._ATTRIBUTE_IDENTIFIER and key not in valid_attributes:  # noqa: E501
            raise MlflowException(
        ...
        if identifier_type == cls._METRIC_IDENTIFIER:
            if token.ttype not in cls.NUMERIC_VALUE_TYPES:
                raise MlflowException(
                    ...
                    error_code=INVALID_PARAMETER_VALUE)
            return token.value
        elif identifier_type == cls._PARAM_IDENTIFIER or identifier_type == cls._TAG_IDENTIFIER:  # noqa: E501
            ...
            raise MlflowException(
                ...
                '{value}'.format(
                    identifier_type=identifier_type, value=token.value),
                error_code=INVALID_PARAMETER_VALUE)
    ...
    def _validate_comparison(cls, tokens):
        ...
        if not isinstance(tokens[2], Token) and \
                (tokens[2].ttype not in cls.STRING_VALUE_TYPES.union(cls.NUMERIC_VALUE_TYPES) or  # noqa: E501
    ...
    def _get_comparison(cls, comparison):
        ...
        comp['value'] = cls._get_value(
            comp.get('type'), stripped_comparison[2])
        return comp
    ...
    def parse_order_by(cls, order_by):
        try:
            ...
            raise MlflowException(
                "Error on parsing order_by clause '%s'" % order_by,
                error_code=INVALID_PARAMETER_VALUE)
    ...
    def sort(cls, runs, order_by_list):
        """Sorts a set of runs based on their natural ordering and an
        overriding set of order_bys.

        Runs are naturally ordered first by start time descending, then by run
        id for tie-breaking.
        """
        runs = sorted(
            runs, key=lambda run: (-run.info.start_time, run.info.run_uuid))
        if not order_by_list:
            return runs
        # NB: We rely on the stability of Python's sort function, so that we
        # can apply the ordering conditions in reverse order.
    ...
    def parse_start_offset_from_page_token(cls, page_token):
        # Note: the page_token is expected to be a base64-encoded JSON that
        # looks like { "offset": xxx }. However, this format is not stable, so
        # it should not be relied upon outside of this method.
        if not page_token:
            return 0
    ...
    def create_page_token(cls, offset):
        return base64.b64encode(json.dumps({'offset': offset}).encode('utf-8'))  # noqa: E501

    @classmethod
    def paginate(cls, runs, page_token, max_results):
        """Paginates a set of runs based on an offset encoded into the
        page_token and a max results limit.

        Returns a pair containing the set of paginated runs, followed by an
        optional next_page_token if there are further results that need to be
        returned.
        """
    ...
    # TODO: Tech debt. Refactor search code into common utils, tracking server
    #       , and model registry specific code.

    VALID_SEARCH_KEYS_FOR_MODEL_REGISTRY = set(
    ...
    def _get_comparison_for_model_registry(cls, comparison):
        ...
        if key not in cls.VALID_SEARCH_KEYS_FOR_MODEL_REGISTRY:
            raise MlflowException(
                "Invalid attribute key '{}' specified. Valid keys "
                " are '{}'".format(key,
                                   cls.VALID_SEARCH_KEYS_FOR_MODEL_REGISTRY))
        ...
        comp = {
            ...
            'value':
            cls._strip_quotes(value_token.value, expect_quoted_value=True)
        }
        return comp
    ...
    def parse_filter_for_model_registry(cls, filter_string):
        if not filter_string or filter_string == '':
            return []
        expected = "Expected search filter with single comparison operator.\
        e.g. name='myModelName'"
        try:
            parsed = sqlparse.parse(filter_string)
        except Exception:
            raise MlflowException(
        ...
        if len(invalids) > 0:
            ...
            raise MlflowException(
                ...
                error_code=INVALID_PARAMETER_VALUE)
        return [
            cls._get_comparison_for_model_registry(si)
