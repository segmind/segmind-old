...
# TODO: this should be restricted to just Git repos and not S3 and stuff
# like that
...
def _is_local_uri(uri):
    """Returns True if the passed-in URI should be interpreted as a path on the
    local filesystem."""
...
def _is_valid_branch_name(work_dir, version):
    """Returns True if the ``version`` is the name of a branch in a Git
    project.
    ...
    if version is not None:
        ...
        repo = Repo(work_dir, search_parent_directories=True)
        try:
            return repo.git.rev_parse('--verify',
                                      'refs/heads/%s' % version) != ''
        except GitCommandError:
            return False
    return False
