...
from segmind_track.lite_extensions.client_utils import (
    _EXPERIMENT_ID_ENV_VAR, _RUN_ID_ENV_VAR, _get_experiment_id, _runid_exists)
# from segmind_track.tracking import artifact_utils
from segmind_track.tracking.client import MlflowClient
from segmind_track.tracking.context import registry as context_registry
from segmind_track.utils import env
from segmind_track.utils.mlflow_tags import (MLFLOW_PARENT_RUN_ID,
                                             MLFLOW_RUN_NAME)
...
def start_run(run_name=None, nested=False):
    ...
    :param run_name: Name of new run (stored as a ``segmind_track.runName`` tag).
    ...
    ))  #if isinstance(experiment_id, int) else experiment_id
    ...
    if len(_active_run_stack) > 0 and not nested:
        raise Exception((
            ...
            + 'current run with segmind_track.end_run(). To start a nested ' +
...
#     """Get the absolute URI of the specified artifact in the currently active
#     run. If `path` is not specified, the artifact root URI of the currently
#     active run will be returned; calls to ``log_artifact`` and
#     ``log_artifacts`` write artifact(s) to subdirectories of the artifact root
#     URI.
