"""The ``segmind_track.projects`` module provides an API for running MLflow
projects locally or remotely."""
...
# import segmind_track.projects.databricks
#import segmind_track.tracking as tracking
from segmind_track import tracking
from segmind_track.entities import RunStatus, SourceType
from segmind_track.exceptions import ExecutionException, MlflowException
from segmind_track.projects import _project_spec
from segmind_track.projects.submitted_run import (LocalSubmittedRun,
                                                  SubmittedRun)
from segmind_track.projects.utils import (_GIT_URI_REGEX, _expand_uri,
...
#import segmind_track.tracking.fluent as fluent
from segmind_track.tracking import fluent
from segmind_track.tracking.context.default_context import _get_user
from segmind_track.tracking.context.git_context import _get_git_commit
from segmind_track.tracking.fluent import _get_experiment_id
from segmind_track.utils import file_utils, process
from segmind_track.utils.file_utils import (path_to_local_file_uri,
                                            path_to_local_sqlite_uri)
from segmind_track.utils.mlflow_tags import (
    LEGACY_MLFLOW_GIT_BRANCH_NAME, LEGACY_MLFLOW_GIT_REPO_URL,
    MLFLOW_DOCKER_IMAGE_ID, MLFLOW_DOCKER_IMAGE_URI, MLFLOW_GIT_BRANCH,
    MLFLOW_GIT_COMMIT, MLFLOW_GIT_REPO_URL, MLFLOW_PARENT_RUN_ID,
    MLFLOW_PROJECT_BACKEND, MLFLOW_PROJECT_ENTRY_POINT, MLFLOW_PROJECT_ENV,
    MLFLOW_SOURCE_NAME, MLFLOW_SOURCE_TYPE, MLFLOW_USER)
# from segmind_track.utils.uri import get_db_profile_from_uri, is_databricks_uri
...
def _run(uri,
    ...
    #     from segmind_track.projects.databricks import run_databricks
    ...
    if backend == 'local' or backend is None:
        ...
        from segmind_track.projects import kubernetes as kb
        tracking.MlflowClient().set_tag(active_run.info.run_id,
                                        MLFLOW_PROJECT_ENV, 'docker')
        tracking.MlflowClient().set_tag(active_run.info.run_id,
...
def run(uri,
    ...
    :raises: :py:class:`segmind_track.exceptions.ExecutionException` If a run launched in blocking mode
             is unsuccessful.
    ...
    :param backend_config: A dictionary, or a path to a JSON file (must end in '.json'), which will
                           ...
                           at https://www.segmind_track.org/docs/latest/projects.html.
    ...
    :param run_id: Note: this argument is used internally by the MLflow project APIs and should
                   ...
                   creating a new run.
    :return: :py:class:`segmind_track.projects.SubmittedRun` exposing information (e.g. run ID)
    ...
    # if backend == 'databricks':
    #     segmind_track.projects.databricks.before_run_validations(
    #         segmind_track.get_tracking_uri(), backend_config)
...
def _fetch_zip_repo(uri):
    ...
    # ```segmind_track.data.download_uri()`` when the API supports the same set of available stores as
...
#     return 'mlflow-%s' % hashlib.sha1(
#         conda_env_contents.encode('utf-8')).hexdigest()


# def _get_conda_bin_executable(executable_name):
#     """Return path to the specified executable, assumed to be discoverable
#     within the 'bin' subdirectory of a conda installation.

#     The conda home directory (expected to contain a 'bin' subdirectory) is
#     configurable via the ``segmind_track.projects.MLFLOW_CONDA_HOME``
#     environment variable. If ``segmind_track.projects.MLFLOW_CONDA_HOME`` is
#     unspecified, this method simply returns the passed-in executable name.
#     """
...
#                             or 'MLFLOW_CONDA_HOME' in os.environ):
#         conda_path = _get_conda_bin_executable('conda')
#         activate_conda_env = [
...
#         with open(os.path.join(dst_path, _GENERATED_DOCKERFILE_NAME),
#                   'w') as handle:
...
#     """Build a docker image containing the project in `work_dir`, using the
#     base image."""
...
#             tag=image_uri,
#             forcerm=True,
#             dockerfile=posixpath.join(_PROJECT_TAR_ARCHIVE_NAME,
...
#     """Returns an appropriate Docker image URI for a project based on the git
#     hash of the specified working directory.
...
#     envs = {
#         'MLFLOW_HDFS_DRIVER':
#         os.environ.get('MLFLOW_HDFS_DRIVER'),
#         'MLFLOW_KERBEROS_TICKET_CACHE':
#         os.environ.get('MLFLOW_KERBEROS_TICKET_CACHE'),
#         'MLFLOW_KERBEROS_USER':
#         os.environ.get('MLFLOW_KERBEROS_USER'),
#         'MLFLOW_PYARROW_EXTRA_CONF':
#         os.environ.get('MLFLOW_PYARROW_EXTRA_CONF')
#     }
#     envs = dict((k, v) for k, v in envs.items() if v is not None)
