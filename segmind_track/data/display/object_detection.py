...
def plot_coco_instance(image_array, bboxes, labels):
    ...
    Args:
        image_array (numpy.ndarray): Description
        bboxes (numpy.ndarray): (N,4) normalized bboxes in (x1,y1,width,height)
        labels (numpy.ndarray): Description
    """
    assert isinstance(bboxes, np.ndarray) and bboxes.ndim==2 and bboxes.shape[1]==4, f"bboxes should be numpy of dimension (Nx4), got {bboxes.shape}"
...
def annotate_coco_dataset(images_dir, annotation_file, result_dir, ignore_missing=False):
...
def plot_voc_instance(image_array, bboxes, labels):
    ...
    Args:
        image_array (numpy.ndarray): Description
        bboxes (numpy.ndarray): (N,4) bboxes in (x1,y1,x2,y2)
        labels (numpy.ndarray): Description
    
    Returns:
        TYPE: Description
    """
    assert isinstance(image_array, np.ndarray), f"image array should be numpy"
    assert isinstance(bboxes, np.ndarray) and bboxes.ndim==2 and bboxes.shape[1]==4, f"bboxes should be numpy of dimension (Nx4)"
    assert isinstance(labels, np.ndarray), f"labels should be numpy of dimesnion (N,)"
    scores = np.ones_like(labels, dtype='float32')

    return annotate_image(image_array, bboxes, scores, labels)

def annotate_voc_dataset(images_dir, annotation_dir, result_dir, ignore_missing=False):
...
def plot_yolo_instance(image_array, bboxes, labels):
    ...
    Args:
        image_array (numpy.ndarray): Description
        bboxes (numpy.ndarray): Description
        labels (numpy.ndarray): Description
    """
    assert isinstance(bboxes, np.ndarray) and bboxes.ndim==2 and bboxes.shape[1]==4, f"bboxes should be numpy of dimension (Nx4)"
...
def annotate_yolo_dataset(images_dir, annotation_dir, result_dir, ignore_missing=False):
    ...
    for imagefile in tqdm(glob(os.path.join(images_dir, '*.*'))):
        ...
        label_name, x_center, y_center, relative_width, relative_height,  = [],[],[],[],[]
        ...
        # image_array = np.array(pil_image)
        bboxes = np.array([x_center,y_center,relative_width,relative_height], dtype='float32').T
        labels = np.array(label_name)
...
def plot_kitti_instance(image_array, bboxes, labels):
    ...
    Args:
        image_array (numpy.ndarray): Description
        bboxes (numpy.ndarray): (N,4) bboxes in (y1,x1,y2,x2)
        labels (numpy.ndarray): Description
    """
