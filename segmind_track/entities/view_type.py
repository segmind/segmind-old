from segmind_track.protos import service_lite_pb2
...
class ViewType(object):
    ...
    def from_string(cls, view_str):
        if view_str not in cls._STRING_TO_VIEW:
            raise Exception(
                ...
                'Valid view types are %s' %
                (view_str, list(cls._STRING_TO_VIEW.keys())))
    ...
    def to_string(cls, view_type):
        if view_type not in cls._VIEW_TO_STRING:
            raise Exception(
                'Could not get valid view type corresponding to string %s. '
                'Valid view types are %s' %
                (view_type, list(cls._VIEW_TO_STRING.keys())))
    ...
    def from_proto(cls, proto_view_type):
        if proto_view_type == service_lite_pb2.ACTIVE_ONLY:
            ...
            return cls.ALL
        raise ValueError(
            'Unexpected proto_view_type: {}'.format(proto_view_type))
