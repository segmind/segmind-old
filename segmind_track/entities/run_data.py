...
from segmind_track.protos.service_lite_pb2 import Param as ProtoParam
from segmind_track.protos.service_lite_pb2 import RunData as ProtoRunData
from segmind_track.protos.service_lite_pb2 import RunTag as ProtoRunTag
...
class RunData(_MLflowObject):
    ...
    def metrics(self):
        ...
        For each metric key, the metric value with the latest timestamp is
        returned. In case there are multiple values with the same latest
        timestamp, the maximum of these values is returned.
        """
        return self._metrics

    @property
    def params(self):
        """Dictionary of param key (string) -> param value for the current
        run."""
        return self._params
    ...
    def to_proto(self):
        ...
        run_data.params.extend([
            ProtoParam(key=key, value=val) for key, val in self.params.items()
        ])
        run_data.tags.extend([
            ProtoRunTag(key=key, value=val) for key, val in self.tags.items()
        ])
        return run_data
