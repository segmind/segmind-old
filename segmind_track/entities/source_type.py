class SourceType(object):
    ...
    :py:class:`cral.tracking.entities.Run`."""
    ...
    SOURCETYPE_TO_STRING = {
        value: key
        for key, value in _STRING_TO_SOURCETYPE.items()
    }
    ...
    def from_string(status_str):
        if status_str not in SourceType._STRING_TO_SOURCETYPE:
            raise Exception(
                ...
                'status strings: %s' %
                (status_str, list(SourceType._STRING_TO_SOURCETYPE.keys())))
    ...
    def to_string(status):
        if status not in SourceType.SOURCETYPE_TO_STRING:
            raise Exception(
                ...
                'statuses: %s' %
                (status, list(SourceType.SOURCETYPE_TO_STRING.keys())))
