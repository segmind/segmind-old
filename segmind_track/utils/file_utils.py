...
from segmind_track.entities import FileInfo
from segmind_track.exceptions import MissingConfigException
...
def list_all(root, filter_func=lambda x: True, full_path=False):
    ...
    matches = [
        x for x in os.listdir(root) if filter_func(os.path.join(root, x))
    ]
    return [os.path.join(root, m) for m in matches] if full_path else matches


def list_subdirs(dir_name, full_path=False):
    """Equivalent to UNIX command: ``find $dir_name -depth 1 -type d``
...
def list_files(dir_name, full_path=False):
    """Equivalent to UNIX command: ``find $dir_name -depth 1 -type f``
...
def make_containing_dirs(path):
    """Create the base directory for a given file path if it does not exist;
...
def write_yaml(root, file_name, data, overwrite=False, ignore_if_exists=False):
    ...
    try:
        with codecs.open(
                yaml_file_name, mode='w', encoding=ENCODING) as yaml_file:
            yaml.safe_dump(
                data, yaml_file, default_flow_style=False, allow_unicode=True)
...
class TempDir(object):
    ...
    def path(self, *path):
        return os.path.join('./', *path) if self._chdr else os.path.join(
            self._path, *path)


def read_file_lines(parent_path, file_name):
    """Return the contents of the file as an array where each element is a
    separate line.
...
def get_relative_path(root_path, target_path):
    """Remove root path common prefix and return part of `path` relative to
    `root_path`.
    ...
    if len(root_path) > len(target_path):
        raise Exception("Root path '%s' longer than target path '%s'" %
...
def _copy_project(src_path, dst_path=''):
    ...
    assert os.path.isfile(os.path.join(
        src_path, 'setup.py')), 'file not found ' + str(
            os.path.abspath(os.path.join(src_path, 'setup.py')))
    shutil.copytree(
        src_path,
        os.path.join(dst_path, mlflow_dir),
...
def get_local_path_or_none(path_or_uri):
    """Check if the argument is a local path (no scheme or file:///) and return
    local path if true, None otherwise."""
    parsed_uri = urllib.parse.urlparse(path_or_uri)
    if len(parsed_uri.scheme) == 0 or parsed_uri.scheme == 'file' and len(
            parsed_uri.netloc) == 0:
