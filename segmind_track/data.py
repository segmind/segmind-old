...
from segmind_track.utils import process
...
def _fetch_dbfs(uri, local_path):
    print('=== Downloading DBFS file %s to local path %s ===' %
...
def _fetch_gs(uri, local_path):
    ...
    print('=== Downloading GCS file %s to local path %s ===' %
          (uri, os.path.abspath(local_path)))
    (bucket, gs_path) = parse_gs_uri(uri)
    storage.Client().bucket(bucket).blob(gs_path).download_to_filename(
        local_path)
